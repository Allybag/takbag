...
struct EngineStats
{
    ...
    void reset() { mEvaluatedNodes = mTerminalNodes = mSeenNodes = mTableHits =  0; }
};

inline std::ostream& operator<<(std::ostream& stream, EngineStats stats)
{
    stream << "Node counts: Seen(" << stats.mSeenNodes << "), Evaluated(" << stats.mEvaluatedNodes
           << "), Terminal(" << stats.mTerminalNodes << "), TableHits(" << stats.mTableHits << ")";
...
public:
    explicit Engine(EngineOptions options = EngineOptions()) : mUseAlphaBeta(options.mUseAlphaBeta),
        mUseMoveOrdering(options.mUseMoveOrdering), mUseTranspositionTable(options.mUseTranspositionTable),
        mMaxDepth(options.mMaxDepth), mOpeningBook(options.mOpeningBookPath), mEvaluator(options.mEvaluator) { }
    ...
    const EngineStats& getStats() { return mStats; }
};
