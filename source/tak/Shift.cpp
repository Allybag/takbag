...
std::size_t applyShift(std::size_t oldIndex, std::size_t size, Shift shiftType)
{
    ...
    switch (shiftType) {
        case Shift::Identical:              return recompose(rowIndex, colIndex);
        case Shift::Vertical:               return recompose(flip(rowIndex), colIndex);
        case Shift::Horizontal:             return recompose(rowIndex, flip(colIndex));
        case Shift::MainDiagonal:           return recompose(colIndex, rowIndex);
        case Shift::OffDiagonal:            return recompose(flip(colIndex), flip(rowIndex));
