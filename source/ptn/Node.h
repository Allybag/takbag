...
struct Node
{
    ...
    Node(Token tagKey, Token tagData)
        : mType(NodeType::TagNode), mFirstToken(std::move(tagKey)), mSecondToken(std::move(tagData))
    {
    }
    Node(std::size_t turnNum, Token firstMove)
        : mType(NodeType::TurnNode), mTurnNum(turnNum), mFirstToken(std::move(firstMove))
    {
    }
    Node(std::size_t turnNum, Token firstMove, Token secondMove)
        : mType(NodeType::TurnNode), mTurnNum(turnNum), mFirstToken(std::move(firstMove)),
          mSecondToken(std::move(secondMove))
    {
    }
